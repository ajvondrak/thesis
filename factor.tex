...
in the \factor|sets| vocabulary.  \factor|set| is a \term{mixin} class---a
...
\factor|USING: ...  ;|
%
form specifies vocabularies being used (like Java's \mint{java}|import|), and
\factor|IN:| specifies the vocabulary in which the definitions appear (like
Java's \mint{java}|package|).  We can see here that instances of the
...
\factor|set| instance, which simply uses \factor|members| to obtain an
equivalent \factor|sequence| of set members, then calls \factor|length|.

By viewing a class as a set of objects that respond positively to the class
