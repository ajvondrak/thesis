...
\factor|{ [ a ] [ b ] } spread|
%
invokes \factor|b| on the top element, and \factor|a| on the element beneath
...
\vref{lst:spread}, the \factor|without-bi*| word shows the simple \factor|dip|
pattern that \factor|bi*| encapsulates.  Here, we're converting the string
\factor|str1| (the second element from the top) into uppercase and
\factor|str2| (the top element) to lowercase.  In \factor|with-bi*|, the
...
\factor|without-tri*|.  In comparison, \factor|with-tri*| is less nested and
...
demonstrated clearly in \vref{lst:apply}.  In \factor|without-bi@|, we see that
...
Thus, \factor|with-bi@| cuts down on the duplicated \factor|[ sq ]| in
\factor|without-bi@|.  Similarly, we can replace the three repeated quotations
passed to \factor|tri*| in \factor|without-tri@| with a single instance passed
to \factor|tri@| in \factor|with-tri@|.  Like other data flow combinators, we
have the numbered variants.  \factor|2bi@| has the stack effect
%
\factor|( w x y z quot -- )|,
%
where \factor|quot| expects two inputs, and is thus applied to \factor|w| and
\factor|x| first, then to \factor|y| and \factor|z|.  Similarly, \factor|2tri@|
