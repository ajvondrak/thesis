...
implemented in the vocabulary shown in \cref{lst:value-numbering-graph}.  This
...
Other definitions in \cref{lst:value-numbering-graph} manipulate expressions
...
\cref{lst:process-instruction}.  The default behavior for dispatching on an
...
\Verb|exprs>vns|, and \Verb|vns>insns|.
...
implementation, we'll examine the example from \cref{fig:value-numbering} in
depth.  For convenience, the before/after snapshot of the \gls{CFG} is
reproduced in \cref{fig:lvn}.
...
Next, block $2$ in \cref{fig:lvn} is processed.  The tables are all reset, so
...
\cref{fig:finalize-lvn}.  Because the value numbering algorithm was local, the
...
\Verb|1| on each loop.  In terms of the \gls{CFG} in \cref{fig:finalize-lvn},
