\subsection{Results}\label{sec:vn:results}

\todo[inline]{Move the ``list of shit I did'' to the intro?}
...
\begin{itemize}
  \item A \factor|graphviz| vocabulary, which provides bindings to create and
        manipulate graphs in Factor and output them using Graphviz\todo{cite?}.
  \item The \factor|compiler.cfg.graphviz| vocabulary, which uses the Graphviz
        bindings to output illustrations of Factor's low-level \glsplural{CFG}.
